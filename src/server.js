...
routerProducts.post('/',authMiddleware, async (req,res, next) => {
    ...
    newProductId
        ? res.status(200).json({"success" : "producto agregado con el ID: "+newProductId})
        : res.status(400).json({"error": "key invalida. Verifique el body del contenido"})
})
...
routerProducts.delete('/:id', authMiddleware, async (req, res, next) => {
    const {id} = req.params;
    const fueEliminado = await contenedor.deleteById(id);
...
routerCart.post('/', async(req, res) => {
    ...
    newCartId
        ? res.status(200).json({"success" : "cart agregada con el ID: "+newCartId})
        : res.status(400).json({"error": "invalid key. Verifique el body del contenido"})
...
routerCart.delete('/:id', async (req, res) => {
    const {id} = req.params;
    const fueEliminado = await carrito.deleteById(id);
    
    fueEliminado 
        ? res.status(200).json({"success": "cart eliminada"})
        : res.status(404).json({"error": "cart no encontrada"})
})
...
routerCart.post('/:id/productos', async(req,res) => {
    ...
    if (product) {
        ...
        res.status(404).json({"error": "product no encontrado, verifique si el id en el body es el correcto."})
    }
})
